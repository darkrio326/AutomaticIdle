...
### 变更
- [2026-04-02 +0800] 小修改（工具系统）：修复刷新页面后工具升级等级丢失问题（`flowStore.buildInitialState` 回写存档时补上 `toolLevels`）；并新增低阶升级门禁：若已购更高阶且覆盖同配方链路的工具（如已购铜镐覆盖采矿链路），则禁用低阶工具升级并给出提示，避免无意义消耗。
- [2026-04-02 +0800] v0.3.0 / ITER-039（发布门禁收口）：新增 `docs/versions/v0.3-regression-baseline.md` 与 `docs/versions/v0.3-RELEASE.md`，回填 v0.3 最小回归命令、门禁判定、风险结论与回滚方案；`v0.3-plan.md` 同步将 ITER-039 标记为 DONE，完成 `ITER-031 ~ ITER-039` 全链路收口。
- [2026-04-02 +0800] v0.3.0 / ITER-038（调优脚本）：新增 `scripts/tune-balance.mjs` 与 `npm run tune:balance`，可对多条典型流程做 20 分钟仿真并输出 `reports/tuning/latest-balance-report.md` / `.json` 报告；报告覆盖净收益、停工率、订单完成率（含高价值订单完成率）和回本时间，用于指导维护费与订单奖励区间调参。
- [2026-04-02 +0800] v0.3.0 / ITER-037（订单增强 v1）：订单生成新增“当前阶段偏置”，会优先偏向当前已解锁、库存已有或当前流程正在产出的资源链；订单区新增“高价值”标识与建议提示，会明确告诉玩家“可直接提交 / 继续当前流程 / 建议切到某条生产链 / 当前阶段尚未解锁”，在不强制打断流程的前提下提高订单牵引力；同时扩展 `ordersTemplate.json`，补充高价值铁锭单与铜矿批量单。
- [2026-04-02 +0800] v0.3.0 / ITER-036（经济压力 v1）：为建筑新增持续维护费（金币 / 秒），运行时按真实时间稳定扣减并将收益口径切换为“净收益”；当维护费导致负收益或金币即将见底时，`StatusPanel.vue` 会显示低余额/负收益预警；若金币不足以支付维护费，runtime 会明确提示“维护费不足”并自动停机，避免出现运行中静默破产。
- [2026-04-02 +0800] 小修改（文案）：收益历史文案从“本次峰值 / 本地最佳 / 轮次最佳”调整为“本次会话峰值 / 本机历史最佳 / 当前轮次最佳”，同步更新运行页与移动端顶部摘要，降低口径歧义。
- [2026-04-02 +0800] v0.3.0 / ITER-035（存档导入导出 v1）：`saveService.ts` 新增手动存档字符串导出与导入解析能力（`AID1` 前缀、轻量编码、完整性校验）；导入链路增加格式/版本/完整性校验，失败会给出明确错误；`StatusPanel.vue` 在“再来一轮”区新增“存档导入导出”入口，支持导出一键复制、粘贴导入与覆盖前确认，避免静默覆盖当前进度。
- [2026-04-02 +0800] 小修改（状态面板）：右侧 `StatusPanel` 统一区块标题样式与显示顺序，调整为「库存 → 订单 → 技能 → 工具 → 建筑 → 再来一轮」；除库存外其余区块均支持折叠/展开。为避免标题样式割裂，`ToolPanel.vue` 与 `BuildingPanel.vue` 新增嵌入模式，由 `StatusPanel` 统一渲染标题与折叠交互。
- [2026-04-02 +0800] v0.3.0 / ITER-034（Prestige v1）：新增技术点重开循环，按 `techPointGain = floor(6 * log2(bestStableGps / 5 + 1))` 计算本轮可得技术点；增加总技术点持久化与全局效率倍率（每点 +2%）并接入 runtime 耗时；`StatusPanel.vue` 增加重开预览文案与重开按钮，重开后重置本轮进度并保留累计技术点。
- [2026-04-02 +0800] v0.3.0 / ITER-033（收益历史与差值）：新增 `sessionBestGps`、`bestStableGps`、`localBestGps` 与当前差值统计；将 `bestStableGps` / `localBestGps` 接入存档；`ExecutionView.vue` 与 `PlayPage.vue` 新增本次峰值、本地最佳、轮次最佳和“与最佳差值”展示，为后续 Prestige 口径打底。
- [2026-04-02 +0800] v0.3.0 / ITER-032（runtime 回归基线）：引入 `vitest`，新增 `test` / `test:runtime` 脚本与 `vitest.config.ts`，并补充 `src/core/runtimeEngine.spec.ts`，覆盖步骤推进、循环边界、资源不足停机与 `pendingFlow` 生效时机四类 runtime core 行为。
- [2026-04-02 +0800] v0.3.0 / ITER-031（runtime 可观测性）：新增 `DebugPanel.vue`，在 `?debug=1` 下提供运行态摘要、库存快照、暂停/继续与单步推进入口；`runtimeEngine.ts` / `runtimeStore.ts` 同步增加最小单步调试接口，并将原 `ExecutionView.vue` 的零散 debug 行收口为独立面板，默认玩家路径不展示调试 UI。
- [2026-04-02 +0800] 文档准备（v0.3.0）：在 `docs/versions/v0.3-plan.md` 补充“实施启动包”，明确当前开工状态、建议实施顺序，以及 ITER-031 / ITER-032 的执行前检查、最小回归包与回滚方式，作为 v0.3.0 正式开工前的执行基线。
- [2026-04-01 +0800] 小修改（技能面板）：技能成长卡片展开明细现在只显示当前已解锁配方，不再提前展示未解锁内容。
- [2026-04-01 +0800] 小修改（技能面板）：右侧“技能成长”卡片支持点击展开明细，展开后显示该技能关联配方的“总加成”列表（如“开采铁矿 总加成 xx%”），并修正技能与配方的关联口径为 `requiredSkillId` 或 `expGains` 命中均纳入。
- [2026-04-01 +0800] 小修改（流程规则）：禁止将流程编辑为“仅售卖步骤”。`flowStore` / `runtimeStore` 新增统一校验，旧存档若只包含售卖步骤会在加载时自动清空该流程；`FlowEditor.vue` 同步禁用“首个步骤直接添加售卖”并显示提示，避免把瞬时售卖操作当作独立循环主流程。
- [2026-04-01 +0800] 修复（移动端）：流程页上下分隔条从仅支持鼠标拖拽改为使用 Pointer Events，手机与触屏设备现在可以正常拖动“步骤区 / 添加步骤区”高度。
- [2026-04-01 +0800] 小修改（移动端）：将“实时收益 / 运行状态”从运行页内部提取为手机顶部固定信息条，切换流程/运行/状态标签时持续可见；运行页内对应顶部信息在移动端隐藏，减少重复展示。
- [2026-04-01 +0800] 小修改（UX）：默认隐藏中部执行区 `debug` 工具条，仅在路由查询参数带 `debug=1` 时显示，避免公开版本默认暴露调试操作。
